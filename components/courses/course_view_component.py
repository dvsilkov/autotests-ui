...
class CourseViewComponent(BaseComponent):
    """
    Класс описывает элементы в компоненте CourseViewComponent (карточка круса на странице)
    Этот компонент можно будет многократно использовать в различных автотестах, для проверки курсов
    """
    def __init__(self, page: Page):
        super().__init__(page)

        # создаем экземпляр для компонента CourseViewMenuComponent
        self.menu = CourseViewMenuComponent(page)

        # локаторы для CourseViewComponent
    ...
    def check_visible(
        ...
        """
        метод проверяет отображение карточки курса
        и корректность данных (заголовок, макс и мин балл, оценочное время)
        """
